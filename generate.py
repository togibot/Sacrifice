...
def hat(t, amp=0.12, open_hat=False):
    ...
    for j in range(n):
        x = j / SR
        noise = random.uniform(-1, 1)
        add_sample(t + x, amp * noise * env_decay(x, decay))
...
def synth_note(t, dur, hz, amp, kind="saw", cutoff=2200, attack=0.008, release=0.12):
    n = int(dur * SR)
    phase = 0.0
    last = 0.0
...
def freq(note):
    ...
    # Accept flats too, fixing the original Bb/Ab bug.
    aliases = {"Db":"Cs", "Eb":"Ds", "Gb":"Fs", "Ab":"Gs", "Bb":"As"}
    if note[:-1] in aliases:
        note = aliases[note[:-1]] + note[-1]
    letter = note[:-1]
    octave = int(note[-1])
    midi = 12 * (octave + 1) + names[letter]
    return 440.0 * 2 ** ((midi - 69) / 12)

def pad_chord(t, dur, notes, amp=0.055):
    for note in notes:
        synth_note(t, dur, freq(note), amp / len(notes), "saw", 1100, 0.15, 0.35)
...
for bar in range(TOTAL_BARS):
    ...
    if bar < 16:
        for p in range(4):
            synth_note(bt + p * BEAT, BEAT * 0.7, freq(chord[0]) / 2, 0.035, "sine", 500, 0.03, 0.18)
    ...
    if 16 <= bar < 32 or 48 <= bar < 72:
        ...
        for b, hz in enumerate(notes):
            ...
            synth_note(bt + b * BEAT, dur, hz, 0.19 if bar < 48 else 0.25, "saw", 420 if bar < 48 else 600, 0.005, 0.08)
    ...
    if 40 <= bar < 48:
        for step in range(8):
            hz = freq(["D4", "Eb4", "F4", "G4", "A4", "Bb4", "C5", "D5"][step])
            synth_note(bt + step * BEAT / 2, BEAT * 0.25, hz, 0.035 + step * 0.006, "saw", 2200, 0.002, 0.04)
        for h in range(16):
